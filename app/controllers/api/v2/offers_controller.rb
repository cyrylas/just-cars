...
class Api::V2::OffersController < ApplicationController
  ...
  def index
    ...
    offers = Offer.all.order(created_at: :desc).limit(limit + 1).offset(offset)
    ...
    @offers = offers.to_a
    @has_next_page = @offers.size > limit
    @offers.pop if @has_next_page
  end
